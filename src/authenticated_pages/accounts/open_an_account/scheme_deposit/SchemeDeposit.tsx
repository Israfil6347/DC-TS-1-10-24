...
function SchemeDeposit() {
  ...
  const {
    ...
    status: accountOpeningEligibilityResponseStatus,
    message: accountOpeningEligibilityResponseMessage,
    executeCommand: AccountOpeningEligibilityExecuteCommand,
  } = useCommand<string | any>();

  console.log(accountOpeningEligibilityResponseStatus);
  console.log(accountOpeningEligibilityResponseMessage);
  console.log(accountOpeningEligibilityResponseData);
  ...
  const updateOperatorNameHandler = (id: any) => {
    ...
    // updateSchemeDepositState('accountHolder', selected?.PersonId);
    // updateSchemeDepositState('accountHolderName', selected?.FullName);
  ...
  const valueAddHandler = (selected: any) => {
    // updateSchemeDepositState('accountHolder', selected?.value);
    // updateSchemeDepositState('accountHolderName', selected?.label);
    checkeligibleHandler(selected?.value);
    updateOperatorNameHandler(selected?.value);
    if (schemeDepositState?.AccountType === '01') {
      addAccountsHolderHandler(selected?.value);
    }
  };
  ...
  return (
    <div>
      ...
      <MyModal
        ...
        onClose={() => setAccountOpeningEligibilityResponseData(null)}
      >
        <MyDialogueView
          dialogueHeader={
                              ...
                              <div className="w-full md:w-1/2">
                                <div className="grid grid-cols-1 gap-6">
                                  ...
                                  <MyDropdown
                                    ...
                                    error={schemeDepositState?.Errors?.amount}
                                    onChange={(event) => {
                                      // const selected =
                                      //   getDurationAmountsResponseData?.find(
                                      //     (obj: any) => {
                                      //       if (
                                      //         parseInt(event.target.value) ===
                                      //         obj?.DurationId
